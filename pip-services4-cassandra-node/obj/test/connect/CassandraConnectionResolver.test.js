...
suite('CassandraConnectionResolver', () => {
    test('Connection Config', () => __awaiter(void 0, void 0, void 0, function* () {
        let dbConfig = pip_services3_commons_node_1.ConfigParams.fromTuples('connection.host', 'localhost', 'connection.port', 9042, 'connection.database', 'test', 'credential.username', 'cassandra', 'credential.password', 'cassandra');
    ...
    test('Connection Config from URI', () => __awaiter(void 0, void 0, void 0, function* () {
        let dbConfig = pip_services3_commons_node_1.ConfigParams.fromTuples('connection.uri', 'cassandra://cassandra:cassandra@localhost:9042/test');
        let resolver = new CassandraConnectionResolver_1.CassandraConnectionResolver();
        resolver.configure(dbConfig);
        let config = yield resolver.resolve(null);
        assert.isObject(config);
