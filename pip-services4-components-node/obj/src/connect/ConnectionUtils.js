...
class ConnectionUtils {
    ...
    static concat(options1, options2, ...keys) {
        let options = pip_services3_commons_node_1.ConfigParams.fromValue(options1);
    ...
    static rename(options, fromName, toName) {
        ...
        if (toValue != null)
            return options;
        options = pip_services3_commons_node_1.ConfigParams.fromValue(options);
    ...
    static include(options, ...keys) {
        if (keys == null || keys.length == 0)
            return options;
        let result = new pip_services3_commons_node_1.ConfigParams();
        for (let key of options.getKeys()) {
