...
suite('ConfigParams', ()=> {
    ...
    test('Config From String', () => {
        let config = ConfigParams.fromString("Queue=TestQueue;Endpoint=sb://cvctestbus.servicebus.windows.net/;SharedAccessKeyName=RootManageSharedAccessKey;SharedAccessKey=K70UpCUXN1Q5RFykll6/gz4Et14iJrYFnGPlwiFBlow=");
        assert.equal(config.length(), 4);
        assert.equal(config.get("Queue"), "TestQueue");
    });    

    test('Config From Object', () => {
        let value = AnyValueMap.fromTuples(
