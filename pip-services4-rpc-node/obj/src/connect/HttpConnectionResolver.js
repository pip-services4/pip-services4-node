...
class HttpConnectionResolver {
    ...
    validateConnection(correlationId, connection, credential) {
        if (connection == null) {
            throw new pip_services3_commons_node_2.ConfigException(correlationId, "NO_CONNECTION", "HTTP connection is not set");
        }
        ...
        if ("http" != protocol && "https" != protocol) {
            throw new pip_services3_commons_node_2.ConfigException(correlationId, "WRONG_PROTOCOL", "Protocol is not supported by REST connection")
        ...
        if (protocol == "https") {
            ...
            if (credential == null) {
                throw new pip_services3_commons_node_2.ConfigException(correlationId, "NO_CREDENTIAL", "SSL certificates are not configured for HTTPS protocol");
            }
            else {
                ...
                if (credential.getAsNullableString("internal_network") == null) {
                    if (credential.getAsNullableString('ssl_key_file') == null) {
                        throw new pip_services3_commons_node_2.ConfigException(correlationId, "NO_SSL_KEY_FILE", "SSL key file is not configured in credentials");
                    }
                    else if (credential.getAsNullableString('ssl_crt_file') == null) {
                        throw new pip_services3_commons_node_2.ConfigException(correlationId, "NO_SSL_CRT_FILE", "SSL crt file is not configured in credentials");
                    }
                }
            }
        }
    ...
    composeConnection(connections, credential) {
        let connection = pip_services3_commons_node_1.ConfigParams.mergeConfigs(...connections);
