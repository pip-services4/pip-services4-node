...
class ContainerReferences extends ManagedReferences_1.ManagedReferences {
    ...
    putFromConfig(config) {
        for (let i = 0; i < config.length; i++) {
            const componentConfig = config[i];
            let component;
            let locator;
            try {
                ...
                if (componentConfig.type != null) {
                    ...
                    component = pip_services4_commons_node_1.TypeReflector.createInstanceByDescriptor(componentConfig.type);
                }
                ...
                if (component == null) {
                    throw new pip_services4_components_node_2.CreateException("CANNOT_CREATE_COMPONENT", "Cannot create component")
