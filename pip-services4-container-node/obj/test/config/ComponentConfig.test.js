...
suite('ComponentConfig', () => {
    ...
    test('Type', () => {
        ...
        let type = new pip_services4_commons_node_1.TypeDescriptor("new name", null);
        componentConfig.type = type;
        assert.equal(componentConfig.type, type);
    });
    test('Descriptor', () => {
        assert.isUndefined(componentConfig.descriptor);
        let descriptor = new pip_services4_components_node_2.Descriptor("group", "type", "id", "default", "version");
    ...
    test('ConfigParams', () => {
        ...
        let config = pip_services4_components_node_1.ConfigParams.fromTuples("config.key", "key", "config.key2", "key2");
        componentConfig.config = config;
        assert.equal(componentConfig.config, config);
    });
    test('From Empty Config', () => {
        let config = pip_services4_components_node_1.ConfigParams.fromTuples();
        try {
    ...
    test('From Wrong Config', () => {
        let config = pip_services4_components_node_1.ConfigParams.fromTuples("descriptor", "descriptor_name", "type", "type", "config.key", "key", "config.key2", "key2");
        try {
            componentConfig = ComponentConfig_1.ComponentConfig.fromConfig(config);
        }
        catch (ex) {
    ...
    test('From Correct Config', () => {
        let descriptor = new pip_services4_components_node_2.Descriptor("group", "type", "id", "name", "version");
        let type = new pip_services4_commons_node_1.TypeDescriptor("type", null);
        let config = pip_services4_components_node_1.ConfigParams.fromTuples("descriptor", "group:type:id:name:version", "type", "type", "config.key", "key", "config.key2", "key2");
        componentConfig = ComponentConfig_1.ComponentConfig.fromConfig(config);
