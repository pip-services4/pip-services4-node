...
suite('VaultDiscovery', () => {
    ...
    let connections = pip_services4_components_node_1.ConfigParams.fromTuples("connKey1.host", "10.1.1.100", "connKey1.port", "8080", "connKey2.host", "10.1.1.101", "connKey2.port", "8082");
    ...
    test('Resolve connections', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        // Resolve one
        let connection = yield discovery.resolveOne(context, "connKey1");
        assert.equal("10.1.1.100", connection.getHost());
        assert.equal(8080, connection.getPort());
        connection = yield discovery.resolveOne(context, "connKey2");
        assert.equal("10.1.1.101", connection.getHost());
        assert.equal(8082, connection.getPort());
        // Resolve all
        yield discovery.register(null, "connKey1", pip_services4_config_node_1.ConnectionParams.fromTuples("host", "10.3.3.151"));
        let connections = yield discovery.resolveAll(context, "connKey1");
        assert.isTrue(connections.length > 1);
        yield discovery.register(null, "connKey2", pip_services4_config_node_1.ConnectionParams.fromTuples("host", "15.7.7.1", "port", "8053"));
        connections = yield discovery.resolveAll(context, "connKey2");
        assert.isTrue(connections.length > 1);
        yield discovery.register(null, "connKey3", pip_services4_config_node_1.ConnectionParams.fromTuples("host", "7.7.0.0", "port", "8585"));
        connections = yield discovery.resolveAll(context, "connKey3");
        assert.isTrue(connections.length == 1);
    }));
});
