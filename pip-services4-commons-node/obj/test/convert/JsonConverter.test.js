...
suite('JsonConverter', () => {
    test('To Json', () => {
        ...
        let jsonFilter = JsonConverter_1.JsonConverter.toJson(filter);
        assert.equal("{\"Key1\":123,\"Key2\":\"ABC\"}", jsonFilter);
        let array = [123, "ABC"];
        let jsonArray = JsonConverter_1.JsonConverter.toJson(array);
        assert.equal("[123,\"ABC\"]", jsonArray);
        let date = DateTimeConverter_1.DateTimeConverter.toDateTime("1975-04-08T00:00:00.000Z");
        let jsonDate = JsonConverter_1.JsonConverter.toJson(date);
    ...
    test('From Json', () => {
        ...
        let filter = JsonConverter_1.JsonConverter.fromJson(null, "{\"Key2\":\"ABC\",\"Key1\":\"123\"}");
        assert.isObject(filter);
        let array = JsonConverter_1.JsonConverter.fromJson(TypeCode_1.TypeCode.Array, "[123,\"ABC\"]");
        assert.equal(2, array.length);
        let date = DateTimeConverter_1.DateTimeConverter.toDateTime("1975-04-08T00:00:00.000Z");
        let jsonDate = JsonConverter_1.JsonConverter.fromJson(TypeCode_1.TypeCode.DateTime, "\"1975-04-08T00:00Z\"");
    ...
    test('To Json Map', () => {
        ...
        let resultElements = result.value1;
        let resultElement0 = resultElements[0];
