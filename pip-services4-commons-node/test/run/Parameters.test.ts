...
suite('Parameters', ()=> {
    ...
    test('Defaults', () => {
        let result = Parameters.fromTuples(
            "value1", 123,
            "value2", 234
        );
        let defaults = Parameters.fromTuples(
            "value2", 432,
            "value3", 345
        );
    ...
    test('Override Recursive', () => {
        let result = Parameters.fromJson(
            "{ \"value1\": 123, \"value2\": { \"value21\": 111, \"value22\": 222 } }"
        );
        let defaults = Parameters.fromJson(
        ...
        let deepResult = result.getAsMap("value2");
    ...
    test('Assign To', () => {
        let value = new TestClass(null, null);
        let newValues = Parameters.fromJson(
    ...
    test('Get', () => {
        let config = Parameters.fromJson(
            "{ \"value1\": 123, \"value2\": { \"value21\": 111, \"value22\": 222 } }"
        );
        ...
        let boolVal = config.containsKey("value3");
    ...
    test('Set', () => {
        ...
        let subConfig = config.getAsMap("field3");
    ...
    test('From Config', () => {
        ...
        let parameters = Parameters.fromConfig(config);
        assert.equal(2, parameters.length());
        assert.equal("ABC", parameters.get("field2"));

        let value = parameters.getAsMap("field1");
