...
suite('CredentialResolver', () => {
    let RestConfig = pip_services4_components_node_1.ConfigParams.fromTuples("credential.username", "Negrienko", "credential.password", "qwerty", "credential.access_key", "key", "credential.store_key", "store key");
    test('Configure', () => {
    ...
    test('Lookup', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        let credential = yield credentialResolver.lookup(pip_services4_components_node_1.Context.fromTraceId("context"));
        assert.isNull(credential);
        let RestConfigWithoutStoreKey = pip_services4_components_node_1.ConfigParams.fromTuples("credential.username", "Negrienko", "credential.password", "qwerty", "credential.access_key", "key");
        credentialResolver = new CredentialResolver_1.CredentialResolver(RestConfigWithoutStoreKey);
        credential = yield credentialResolver.lookup(pip_services4_components_node_1.Context.fromTraceId("context"));
        ...
        credentialResolver = new CredentialResolver_1.CredentialResolver(RestConfig);
        credentialResolver.setReferences(new pip_services4_components_node_2.References());
        try {
            credential = yield credentialResolver.lookup(pip_services4_components_node_1.Context.fromTraceId("context"));
            assert.fail("Expected to fail");
        }
